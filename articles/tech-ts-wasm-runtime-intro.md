...
TypeScriptを書いていると、普段はメモリ配置をほとんど意識しません。
...
このコードを見たとき、多くの場合は「`p.x` は `10`」「`p.y` は `20`」と考えれば十分です。`Point` のインスタンスがメモリ上のどこに置かれているのか、`x` と `y` が何バイト目にあるのか、`p` という変数が実体を持っているのか参照を持っているのか、といったことを日常的に考える必要はありません。

それはTypeScriptが悪いという話ではなく、むしろ高水準言語としての強さです。メモリ管理やオブジェクト表現はJavaScriptエンジンが引き受けてくれるので、私たちはアプリケーションのロジックに集中できます。

一方で、WebAssemblyを触り始めると話が変わります。WASMの世界では、メモリはもっと素朴に見えます。ざっくり言えば、巨大なバイト配列のような領域があり、その中の何番地に何バイトの値があるか、というモデルで理解できます。

この記事では、TypeScriptに近い構文でWebAssemblyを生成できるAssemblyScriptを題材にして、次のことを見ていきます。
