...
# チームで行うバージョン管理
昔は、USBメモリやGoogle Driveを使ってプロジェクトのファイルを共有し、バージョン管理を行うチームもありました。この方法は、
- 学校の授業での短期間のチーム開発
- 個人での小規模なプロジェクト
といった場合には大きな問題はなく、ある程度スムーズに共同作業を進めることができます。
...
現在いるブランチに、指定したブランチの変更を統合します。
コンフリクト（衝突）が発生した場合は手動で解決する必要があります。
